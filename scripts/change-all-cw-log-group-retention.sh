...
# Get list of all CloudWatch Log Groups in one region where retention is not equal to new default
function ListLogGroups(){
	if [[ $DEBUGMODE = "1" ]]; then
		echo "Begin ListLogGroups Function"
	fi
	ListLogGroups=$(aws logs describe-log-groups --query "logGroups[?retentionInDays != \`$RetentionInDays\`]" --output=json 2>&1)
	if [ ! $? -eq 0 ]; then
		fail "$ListLogGroups"
	# if echo "$ListLogGroups" | egrep -iq "error|not"; then
	# 	fail "$ListLogGroups"
	else
		ParseLogGroups=$(echo "$ListLogGroups" | jq '.[] | .logGroupName' | cut -d \" -f2)
	fi
...
function SetRetentionPolicy(){
	...
	do
		...
		SetRetentionPolicy=$(aws logs put-retention-policy --log-group-name $LogGroup --retention-in-days $RetentionInDays --output=json 2>&1)
